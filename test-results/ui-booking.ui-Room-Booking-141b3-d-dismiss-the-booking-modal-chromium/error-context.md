...
- Name: ui\booking.ui.spec.ts >> Room Booking >> Cancel or close button should dismiss the booking modal
- Location: tests\ui\booking.ui.spec.ts:93:3
...
Locator: locator('button:has-text("Close"), button:has-text("Cancel"), [aria-label*="close"], [aria-label*="Close"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("Close"), button:has-text("Cancel"), [aria-label*="close"], [aria-label*="Close"]').first()
